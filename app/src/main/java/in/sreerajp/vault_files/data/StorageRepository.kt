...
class StorageRepository(
    ...
    /**
     * Attempts to resolve a content:// or file:// URI to a local File on the device.
                ...
                try {
                    ...
                    if (split.size >= 2) {
                        ...
                        if ("primary".equals(type, ignoreCase = true)) {
                            return File(android.os.Environment.getExternalStorageDirectory().toString() + "/" + path)
                        } else {
                            val rawPath = "/storage/$type/$path"
    ...
    /**
     * Cheaply maps a physical File to a FileItem representation.
     */
    suspend fun getSingleFileItem(file: File): FileItem = withContext(Dispatchers.IO) {
        val securedFoldersList = securedFolderDao.getAllSecuredFolders().map { it.path }
        return@withContext mapToFileItem(file, securedFoldersList)
    }
}
