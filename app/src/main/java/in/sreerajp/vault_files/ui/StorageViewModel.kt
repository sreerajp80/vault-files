...
import `in`.sreerajp.vault_files.data.*
...
class StorageViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // File picking state variables
    private val _isPickMode = MutableStateFlow(false)
    val isPickMode: StateFlow<Boolean> = _isPickMode.asStateFlow()

    private val _allowMultiplePick = MutableStateFlow(false)
    val allowMultiplePick: StateFlow<Boolean> = _allowMultiplePick.asStateFlow()

    // Shared flow to emit chosen Uris back to MainActivity
    private val _pickedUris = kotlinx.coroutines.flow.MutableSharedFlow<List<Uri>>()
    val pickedUris = _pickedUris.asSharedFlow()
    ...
    fun startPickMode(allowMultiple: Boolean) {
        _isPickMode.value = true
        _allowMultiplePick.value = allowMultiple
        clearCategoryFilter()
        _currentDirectory.value = userStorageRoot
        loadFilesInDirectory(userStorageRoot)
    }

    fun finishPicking(uris: List<Uri>) {
        viewModelScope.launch {
            _pickedUris.emit(uris)
        }
    }

    fun exitPickMode() {
        _isPickMode.value = false
        _allowMultiplePick.value = false
    }

    fun previewExternalContentUri(context: android.content.Context, uri: Uri) {
        viewModelScope.launch {
            dispatchMessage("Loading external file...")
            val resolvedFile = repository.resolveUriToFile(uri)
            if (resolvedFile != null && resolvedFile.exists()) {
                val item = repository.getSingleFileItem(resolvedFile)
                if (item.isDirectory) {
                    navigateToDirectory(resolvedFile)
                } else if (item.category == "Image" && imagePreviewEnabled.value) {
                    openImagePreview(item)
                } else if (textPreviewEnabled.value && isTextPreviewable(item)) {
                    openTextPreview(item)
                } else if (item.name.lowercase().endsWith(".securenote")) {
                    openNote(item)
                } else {
                    dispatchMessage("Successfully loaded: " + item.name)
                }
                return@launch
            }
            ...
            val tempFile = withContext(kotlinx.coroutines.Dispatchers.IO) {
                try {
                    val displayName = try {
                        ...
                        null
                    } ?: uri.lastPathSegment?.substringAfterLast('/')?.takeIf { it.isNotBlank() } ?: "shared_file"
                    
                    val suffix = if (displayName.contains(".")) "." + displayName.substringAfterLast(".") else ""
                    val prefix = if (displayName.contains(".")) displayName.substringBeforeLast(".") else displayName
                    val cleanPrefix = prefix.take(15).filter { it.isLetterOrDigit() }.ifBlank { "preview" }
                    
                    val file = File.createTempFile(cleanPrefix, suffix, context.cacheDir)
            ...
            if (tempFile != null && tempFile.exists()) {
                val item = repository.getSingleFileItem(tempFile)
                if (item.category == "Image" && imagePreviewEnabled.value) {
                    openImagePreview(item)
                } else if (textPreviewEnabled.value && isTextPreviewable(item)) {
                    openTextPreview(item)
                } else if (item.name.lowercase().endsWith(".securenote")) {
                    ...
                    setPendingSharedImports(listOf(SharedImport(uri, tempFile.name)))
                }
            } else {
                dispatchMessage("Could not open external file.")
            }
        }
    }
}
